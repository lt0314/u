javascript: void(window.jQuery ? (OKfunction(), $('body').attr('ccc', 'ok')) : (function (s, f) {
		var c = document,
		d = c.createElement('script');
		d.type = 'text/javascript',
		d.readyState ? d.onreadystatechange = function () {
			'loadedcomplete'.indexOf(d.readyState) + 1 ? (d.onreadystatechange = null, s && s()) : f && f()
		}
		 : (d.onload = function () {
			s && s()
		}, d.onerror = function () {
		...
		d.src = 'https://cdn.bootcdn.net/ajax/libs/jquery/3.6.0/jquery.min.js',
		c.getElementsByTagName('head')[0].appendChild(d)
	})(function () {
		(OKfunction(), $('body').attr('ccc', 'ok'))
	}, function () {
		document.body.setAttribute('ccc', 'error')
	}))
function OKfunction() {
	javascript: (function ($) {
		var _ = $.prototype;
	...
	javascript: (function (_) {
		_.click = function (doc, p, b, rr) {
			var a,
			c,
			d = $('a,:button,span', doc).filter(':visible').FILTER(b).last()[0];
			d = d ? d : $('a,:button,span', doc).FILTER(b).last()[0];
			d ? d.click() : (a = $('input', doc).FILTER(b), c = a.next(':button'), c[0] ? c.eq(0).click() : (c = a.next().find(':button,a'), c[0] && c.eq(0).click()));
			setTimeout(function () {
				if (rr) {
					rr[0] = 1
				}
		...
		_.setValue = function (doc, p, b, value, rr, tm) {
			var a,
			ne,
			f = $('label', doc).FILTER(b);
			f = f[0] ? f : $('label', p).FILTER(b);
			ne = f.next();
			ne[0] || (a = $('input,textarea', p).FILTER(b), a[0] && a.eq(0).V(value).next().find('a')[0].click());
			if (a && a[0]) {
				if (rr) {
					rr[0] = 1
				};
			...
			if (b.indexOf('附件') + 1) {
				var s = '#attachment,.attach-list',
				fj = f.parent().has(s);
				if (!fj[0]) {
					fj = f.parent().parent().has(s)
				};
			...
			ne = ne[0] ? ne : $('label,span:visible', p).FILTER(b).parent().next();
			ne.find('li')[0] ? setSelectValue(doc, p, b, value, rr) : ne.find('a')[0] && !ne.find('[class*=calendar]')[0] ? setDialogValue(doc, p, b, value, rr, tm || 333) : (a = ne.find('input,textarea'), (a[0] ? a : ne.prev()).V(value), rr & setTimeout(function (rr) {
					rr[0] = 1
				}, 99, rr))
		};
		_.setSelectValue = function (doc, p, b, value, rr) {
			...
			ne = ne[0] ? ne : $('span', doc).FILTER(b).parent().next();
	...
	javascript: (function (_) {
		...
		_.Getit = function (label) {
			var n = label.indexOf('>') + 1,
			r,
			p = $.P(n ? label : ''),
			doc = $('div[id*=_dialog_]:visible', p).TOP()[0] || p,
			L = n ? label.split('>')[1] : label,
			v;
			var d = new DO(),
			lod = 'div:contains(正在处理)';
			d.add(function (r) {
				var id = setInterval1(function (id) {
						!$(lod, doc)[0] && !$(lod, p)[0] && (clearInterval(id), setTimeout(function () {
								r[0] = 1
							}, 1))
					}, function () {
						r[0] = 2
				...
				v = $('label:visible', doc).FILTER(L).next().find('input,textarea')[0] || $('label:visible', p).FILTER(L).next().find('input,textarea')[0];
				v = v ? $(v).val() : (r = $('td:contains(' + label + '):last', doc), r = r[0] ? r : $('td:contains(' + label + '):last', p), v = r.NEAR('td', 'b'), v[0] ? v.text() : '');
				$('body').attr('ccc', v || ' ');
				r[0] = 1
			});
		}
