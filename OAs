javascript: document.body.setAttribute('ccc', 'ok')
javascript: void(window.jQuery ? $('body').attr('ccc', 'ok') : (function (s, f) {
		var c = document,
...
javascript: (function (_) {
	...
	_.click = function (doc, p, b, rr) {
		...
		c,
		vd = $(vdm, p).length,
		d = $('a,:button,span', doc).FILTER(b).last()[0];
		d ? d.click() : (a = $('input', doc).FILTER(b), c = a.next(':button')[0], c ? c.click() : (c = a.next().find(':button,a')[0], c && c.click()));
		setTimeout(function () {
			if (rr) {
				rr[0] = 1
			}
